...
- 切换工程前保存旧工程，失败则保持旧工程；成功切换时清理两端几何、结果与视口，按工程引用恢复几何与当前方案网格。自动保存回调与视口异步读取使用工程纪元隔离。
- 前端 `withBusy` 按操作 ID 跟踪在途动作，嵌套或并发动作各自结束后才释放对应占用；不得让一个动作的完成放开其他动作的工程切换保护。
- 每次求解分配 `cases/<studyId>/<runId>`，活动运行不共享输入目录。持久化方案 ID 只允许 ASCII 字母、数字、下划线和连字符，并要求工程内唯一。
- 作业提升与取消令牌登记必须原子完成。取消请求交给拥有进程的工作线程，确认退出后才标记取消；远端状态无法确认时必须明确报错，不能声称已终止。标准输出与错误输出同时排空。
- 场缓存按路径、mtime 与长度区分文件修订；重扫显式清空，不完整场不缓存。探针只读采样不修改主场/对比槽位。
- 渲染与变形以不可变 Arc 快照运行，锁内不提取边界。KRM1 网格协议为小端 magic、三个 u32 数量和 f32 位置/u32 索引/u32 面所属单元数组；前端校验长度后通过 typed array 视图解码。
...
core 有 100% 行覆盖门槛，以下六种写法会让工具报出**不可达的未覆盖**——它们的共同点是
...
| **多份单态实例**   | 泛型编排函数被两种 runner（作业线程 / 测试替身）各实例化一次：同一源行登记两次，其中提前返回的那份计数为 0，整行被报成未覆盖 | 编排参数取 `&dyn Trait`（或让测试共用同一个实现类型），同一源行只留一份区域 |

跨平台那条尤其隐蔽：本地 macOS 全绿、CI 上 Windows 才红。**错误映射不要按调用点分写。**
多份单态那条同形：只有一种 runner 时满覆盖，多一种测试替身就报未覆盖——**编排不要按调用方复制**。

闭包体是独立的覆盖落点：`unwrap_or_else(|| …)` 这类兜底闭包即使外层每次都跑到，闭包体没执行过
仍会把它所在的行报成未覆盖；按「单侧偏态分支」那条处理（改写成直白写法，或让用例真的走进去）。

门槛没过时 `coverage:rust` 会自行列出落点（按文件分「整行未执行」与「行内有 0 计数区域」两类，
带行号与源码行），不必再手动翻 llvm-cov 的 JSON。
