...
| T96 | [T96-full-flow-e2e.md](T96-full-flow-e2e.md)                                                 | 全流程集成测试（正常使用：几何 → 网格 → case → 求解 → 结果） | **第一步已落地**：L1（无 VM，CI 三端）+ L2（真机 VM，`KAIROS_E2E_VM=1`）+ 编排收口到 `services::vm_run`；L2 首跑即抓出 `detached_launch_command` 引号失衡等两处缺陷。剩余：runner 执行策略入测、失败 / 取消路径、发布前门禁 |
...
5. **T33 可做部分**（P3）：updater 插件接入 + `style-src 'unsafe-inline'` 改造（nonce / 文件化）；
   真机回归与三平台签名属档 B。
6. **T96 全流程集成测试**（P1，进行中）：L1/L2 两层已落地，剩余三项（runner 执行策略入测、
   失败与取消路径、发布前门禁）都是小增量；每完成一项就多一类「只有真机才暴露」的缺陷被挡住。
