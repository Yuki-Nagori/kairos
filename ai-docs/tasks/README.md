...
| ID  | 文件                                                                                         | 任务                                            | 依赖 / 状态                                                                                                                                                                                                    |
| --- | -------------------------------------------------------------------------------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T29 | [T29-real-solve-e2e.md](T29-real-solve-e2e.md)                                               | 真实求解端到端验证（OpenFOAM 环境）             | **已闭环（v1.0.0）**：三条链路 + 4 进程并行（78,125 四面体全链路：退出码 0、2220 步到 endTime、结果回传 + 读场）+ GUI 作业生命周期（拆至 T94）；余视口截图/XY 探针与本地真实件复测（原档已失，改按同规模验收） |
...
| [T99](T99-runtime-and-result-hardening.md) | 待开工 | 生命周期服务、场共享/统计/导出、DTO 与状态治理 |

T98 的本机测试不替代 T94/T96 的 Windows/WSL、Linux 与真实 VM 验收。

## 执行顺序（2026-09-14 核对后确认）

核对范围：全部未完成任务（T29/T33/T48/T60/T82/T83/T89/T91/T92/T93/T94/T95，以及
2026-09-15 新增的 T97）逐条对照
...
5. **T33 可做部分**（P3）：updater 插件接入 + `style-src 'unsafe-inline'` 改造（nonce / 文件化）；
   真机回归与三平台签名属档 B。
6. **T96 全流程集成测试**（P1，**已完成**）：L1/L2 两层 + 编排与执行策略收口 + 失败可见性 + 取消路径；
   首轮就抓出三处只有真机才暴露的缺陷（tar 警告误判、launch 引号失衡、VM 取消不停进程）。
7. **T97 求解环境库重复守卫**（P1，2026-09-15 新增，建议优先）：037 的根因（库被加载两次）
   现在只能靠「跑完才崩」暴露；本任务把它变成提交作业前的显式报错 + 部署前清理。
   证据见 [solver-lib-duplication-report.md](../reviews/solver-lib-duplication-report.md)。
