...
3. **残差报告**：已在 `kairos-core` 增加 Cross-WLF 与 Tait 预测和残差摘要（绝对误差、相对误差、RMSE、`log10(η)` 最大误差与 RMSE）；后续补充逐点 JSON/CSV 导出。
4. **材料资产元数据**：扩展材料 DTO，记录来源类型、单位、拟合算法版本、参数修订号、数据摘要哈希和创建时间，支持版本化而不覆盖内置材料。
5. **CLI 闭环**：增加 `material fit` / `material validate`，并让 Mug/DOE case 按材料 ID 或路径选择自定义材料；当前求解路径仍默认取内置材料，无法完成真正的自定义材料 baseline。
