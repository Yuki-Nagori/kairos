...
当前材料服务只支持已拟合参数的 JSON/CSV 导入导出，尚未形成从曲线到可运行材料的闭环。T101 需要补齐：

1. **曲线解析与单位归一**：在 `kairos-core` 增加 PVT/黏度数据模型、表头解析、单位转换、排序去重、缺失点和非有限值校验。
2. **拟合核心**：提供确定性的 Tait 与 Cross-WLF 拟合服务，输出参数、权重、拟合区间、收敛状态和版本化算法标识；数值计算留在 Rust core。
3. **残差报告**：输出逐点预测、绝对/相对误差、`log10(η)` 误差、最大值和 RMSE，并提供 JSON/CSV 导出与可复核摘要。
