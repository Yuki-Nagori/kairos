...
- WebGPU 顶点和片元着色器独立编译，消除拼接后的重复 WGSL 声明。
- 几何导入/恢复/修复和体网格更新自动刷新视口；重复加载替换旧数据，过期请求不能覆盖新网格。
- 渲染器与网格保留原生对象身份；四分格选择改为可写状态。
- 上传后自动适应模型；两端按包围盒计算远裁剪面，复位保留模型取景，取消缩放距离 500 的上限。
- 新增真实 GPU 浏览器夹具与自动同步回归，执行方法见 [测试地图](../../tests/README.md)。
