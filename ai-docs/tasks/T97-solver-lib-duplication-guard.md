...
- 状态：**进行中**（部署前清理已完成；库布局探测与真实 VM 坏状态验收待下一批）
...
- **库布局守卫（待开工）**：需要在真实 bash 环境中读取 `FOAM_LIBBIN`、
  `FOAM_SITE_LIBBIN`、`FOAM_USER_LIBBIN`，区分实体文件、相对符号链接和硬链接，
  并给出可执行修复提示；完成后再进行 VM 坏状态 / 恢复回归。
