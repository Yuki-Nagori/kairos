...
| `uplot`        | 1.6.32                  | MIT                       | XY 图表缩放、拖拽、探针 series 与报告快照     |
...
| XY 曲线：`utils/chart.ts`、`views/xy-chart/useXyChartPanel.ts`                                                 | 已加入 uPlot 适配边界；现有 Canvas、探针、主题、快照和 CSV 仍在迁移期保留；`bench/chart.ts` 记录 100 万点数据基线     | **P3-a 转为引入 uPlot**。uPlot 提供缩放、平移和大序列交互；先通过适配层迁移，再收口 Canvas 快照与现有交互，避免一次性改动过大  |
...
| XY 图表候选库      | 已引入 uPlot 1.6.32；Canvas 数据基线仍保留（Apple Silicon，本地 Bun）：1M 值域扫描 951,406 ns/op，1M→2K 抽样 1,316,075 ns/op | 已补当前构建包体证据；后续按大数据交互实测决定是否继续拆分 chunk                               |
| uPlot 当前构建     | `bun run build`：JS 595.24 kB（gzip 197.07 kB），CSS 64.27 kB（gzip 15.15 kB）；Vite 保留大 chunk 警告                       | 已有实际构建证据；后续可按功能拆分再优化 chunk，不把历史 Canvas 数字与当前构建混比             |
...
3. **P3-a 图表迁移（进行中）**：已引入 MIT 许可的 uPlot 1.6.32 和独立适配层；下一批迁移 XY 面板的缩放/平移、探针叠加与 Canvas 快照，并补构建包体对照。旧 Canvas 数据基线保留用于回归。
