...
| XY 曲线：`utils/chart.ts`、`views/xy-chart/useXyChartPanel.ts`                            | 仍为自绘 Canvas，chartRange 已统一值域扫描，绘制与探针复用                                                    | **P3-a 评估中**。uPlot 未引入；视觉、交互、性能、包体对照通过后才决定替换，不能预先写死「引库」                                    |
...
| 低层数学：`render/math.ts`、`render/webgpu/volume.ts`                                     | 仍自写 mat4/vec3，volume 已从 math 引入 normalize/cross/sub                                                   | **gl-matrix 尚未采用，取舍待补**。前端消重已完成；库化需验证矩阵约定、精度、性能与包体，不能混入 Rust nalgebra 的完成状态          |
| 色标：`render/webgpu/shaders.ts`、`shaders_volume.ts`、`views/viewport/ViewportPanel.vue` | 表面为蓝红两端插值，体积用另一组蓝橙端点，图例为蓝绿橙三色渐变                                                | **统一尚未实现**，且不只是重复代码，实际颜色口径不一致。继续采用自建常量方案、不引 d3-scale-chromatic；同时检查 WebGL 的对应色映射 |
...
1. **P3-b 补漏**：repair 局部向量原语改用 vec3，保留相交容差与算法，跑 repair / 几何相关用例与共享件消融。
2. **色标统一**：统一表面、体积、图例及 WebGL 对应映射；先确认端点与中间值期望，再验证实际显示。
3. **P3-a 图表评估**：保留当前 Canvas 基线，对比 uPlot 的主题、刻度、探针、截图导出及大序列交互，同时给许可、性能、gzip 增量；结果可以是继续自绘。
4. **前端数学取舍**：为 gl-matrix 给出独立保留/替换结论；若替换，先保留旧基准并对拍矩阵与向量语义。
