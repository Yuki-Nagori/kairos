...
| XY 曲线：`render/uplot-runtime.ts`、`views/xy-chart/useXyChartPanel.ts`                                        | uPlot 已作为唯一绘制路径；探针 series、主题、容器自适应、缩放复位和报告快照均已接入；`utils/chart.ts` 仅保留数据 / CSV 工具 | **P3-a 基础迁移完成**。后续按真实大数据交互继续优化 chunk 和 tooltip，不保留旧 Canvas 自绘实现                                 |
