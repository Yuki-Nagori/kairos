...
| 目标尺寸 | 网格规模      | 求解墙钟                                     |
| -------- | ------------- | -------------------------------------------- |
| 2.0mm    | 625 四面体    | 12.6 秒                                      |
| 1.0mm    | 5,000 四面体  | 32.0 秒                                      |
| 0.5mm    | 40,000 四面体 | 20 分钟以上（跑到 16 分钟才走完约 1/3 步数） |

时间随细化**超线性**增长：单元数增多的同时步长（Co 数受限）变小、步数跟着涨，所以「细一档」的
代价远大于面积比。这条数据是几何面板建议尺寸设 2.0mm 下限的依据（`useGeometryPanel` 的
`MIN_SUGGESTED_SIZE_MM`）；同时说明首轮试用不适合用「最大边 ÷ 20」直接给出的细网格。
