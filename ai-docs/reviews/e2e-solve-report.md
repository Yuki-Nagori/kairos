...
最后更新：2026-09-14（bundle v0.2.5 轮）
...
| 桌面应用作业链路（提交 → VM 复制 → 求解 → 日志回传）                   | ✅ v0.2.5 轮打通（§4.2 缺陷 4 修复后复跑）                |
| 前端视口 / 图表（视觉截图核验）                                        | ⏳ 见 §6（本会话图片栅格不可交付，功能面按 AX 树核验）    |
| 真实件 4 进程并行                                                      | ❌ 求解器死锁（上游 issue #7，已附最小复现 case）         |

**一句话**：Kairos 侧的生成 / 部署 / 提交 / 回传 / 读取全链路可用；剩余问题集中在求解器侧的
真实件多进程并行，以及展示侧的截图核验。
...
| 项             | 值                                                                                                                                                                                                                      |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| bundle（当前） | `moldingFoam-v0.2.5-arm64`，113,869,420 B，sha256 `b36b9f33…`，发布 2026-09-13T15:37Z                                                                                                                                   |
...
| 并行                | —                                                                      | np=4 **死锁**：第 1 个时间步后不再前进，4 rank 常驻 90% CPU、无输出无报错                                                                                      |
...
5. **真实件 4 进程死锁**（未修，issue #7）：见 §3，附最小复现 case。

## 6. 未决与遗留

- **前端视口 / 图表视觉核验**：本会话 Computer Use 的图片栅格不可交付（屏幕录制与辅助功能
  权限均已授予，但模型侧收不到图像），`screencapture` 亦受 TCC 限制。功能面已按 AX 树核验
  （视口 / 图表控件可达），**截图核验待下一次可截图的会话补做**。
- **最小连通性复跑**：当前处于「小 case 快跑」阶段（样例盒 1 mm 体素 / 4 进程，26 s 量级），
  用于日常确认 GUI → VM → 求解 → 结果链路；真实件与 4 万单元网格按需再跑。
- **真实件并行**：求解器修 issue #7 前，真实件用 1–2 进程。
- **真实件工艺参数**：默认工艺对 880 cm³ 件偏激进（体积流量 879 cm³/s 超出常规注塑机
  包络 ≈ 500 cm³/s）；CLI 已给「建议注射时间 ≥ 1.8 s」提示，界面按件设置。
- **（历史）样例 case 填充窗口**：v0.2.1 轮在 endTime 6 s / `maxAlphaCo` 0.03 下的失稳
  现象在 v0.2.5 不复现，记录保留供回溯。

## 7. 最小连通性复现（CLI，5 分钟量级）
