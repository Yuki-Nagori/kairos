...
| 真实件 4 进程并行                                                      | ⏳ 上游已修（issue #7，046，**未发版**）：待新版 bundle 到手后复测       |
...
| 并行                | —                                                                      | np=4 **死锁**：第 1 个时间步后不再前进，4 rank 常驻 90% CPU、无输出无报错。上游定位为「边界 patch 循环内归约」（各 rank patch 数 5/6/5/6 → 归约次数不等）、已修复并加回归用例，**尚未发版** |
...
新用例 `parallelMassBudget` + `run-solver-tests.sh` 并行超时门禁），但**尚未发版**——
Kairos 侧复测等新版 bundle 到手再做。
...
5. **真实件 4 进程死锁**（上游已修、未发版，issue #7）：见 §3。上游 046 已定位根因（边界 patch 循环内归约
   导致各 rank 归约次数不等）并修复 + 加回归防线；Kairos 侧等新版 bundle 发版后复测 np=4。
...
3. **真实件并行**：上游 046 已修 issue #7 但未发版，当前 bundle（v0.2.5）真实件仍只能用 1–2 进程；
   新版 bundle 到手后复测 np=4。
