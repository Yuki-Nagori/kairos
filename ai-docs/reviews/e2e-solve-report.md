...
1. **GUI 提交的作业中途消失且状态不收敛**（新发现，待查）：`foamRun` 在 VM 内启动后数十秒内
   进程消失（无输出、无时间目录），作业在界面上停「运行中」；同一 case、同一命令手动执行可
   跑满 endTime，说明 case 与求解环境正常，问题在桌面端作业进程的生命周期管理。
   下一步：把 spawn 的命令行与子进程退出码/信号写进作业日志（当前日志只有求解器前两行），
   再定位是会话/进程组被回收还是子进程被信号终止。
