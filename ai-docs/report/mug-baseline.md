...
- 本报告中的参考值来自本地受控 Moldflow 报告；原始材料曲线、专有数据库和原始结果文件不进入仓库。
- Kairos 当前使用三维体网格，参考结果使用 Dual Domain；两者只能比较工艺输入、趋势和量级，不能宣称逐点一致。
- 运行记录中的“通过”仅表示命令、case 生成或 solver 进程达到对应验收条件，不表示材料或物理结果已被实验验证。
