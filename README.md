...
当前处于框架搭建阶段：技术栈与工程化设施全部就位，项目管理（新建 / 打开 / 保存 `.kairos` 工程、研究管理、最近项目）与材料数据库（Cross-WLF / Tait 参数模型、内置示例材料、自定义材料导入导出）已落地；网格剖分、各物理场求解器、3D 视口将按总体方案（`ai-docs/tasks/`）逐步落地。
