...
当前处于框架搭建阶段：技术栈与工程化设施全部就位，项目管理（新建 / 打开 / 保存 `.kairos` 工程、研究管理、最近项目）、材料数据库（Cross-WLF / Tait 参数模型、内置示例材料、自定义材料导入导出）、几何导入（STL 健康检查）、3D 体积网格生成（体素 + 5-四面体保形分解）、浇口流道与冷却水路建模、成型工艺设置已落地；各物理场求解器与 3D 视口将按总体方案（`ai-docs/tasks/`）逐步落地。
