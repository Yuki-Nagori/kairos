/**
 * 主题核心（纯 TS，不依赖 Vue）：动态发现 + 注入 + 切换 + 持久化。
 *
 * 主题 = `theme/` 目录下的 CSS 文件名（不含扩展名）。
 * 新增主题 = 丢一个 CSS 文件进来，无需改任何代码。
 * CSS 通过 `[data-theme="名称"]` 选择器定义变量，`<html data-theme="...">` 激活。
 *
 * 放在 utils 而不是 composables：渲染后端要监听主题变化重取清屏色，
 * 而 render 层不许反向依赖 composables（依赖只能向下）。响应式封装见
 * `composables/useTheme.ts`。
 */
import { storageGet, storageKey, storageSet } from "./storage";
...
/**
 * 读文档根的 CSS 变量原文（已 trim）；变量缺失或环境无 getComputedStyle 时用回退值。
 * 调用方需监听 THEME_CHANGED_EVENT 重取——变量在主题切换时才变。
 */
...
/** 颜色变量读成 0..1 的 RGB 分量；值不是 `#rrggbb` 形式（缺失 / 用了 rgb() 等）时返回回退色。 */
export function themeRgb(
  name: string,
  fallback: [number, number, number],
): [number, number, number] {
  const hex = /^#([0-9a-f]{6})$/i.exec(themeVar(name, ""))?.[1];
  if (hex === undefined) {
    return fallback;
  }
