...
/// release 资产名是否匹配目标架构。
///
/// 命名 `moldingFoam-<version>-<arch>.tar.xz`，`<arch>` 为 `arm64` / `amd64`；
/// bundle 一律跑在 Linux 虚拟机里，发布侧也只出这一种包，因此**只按架构判**，
/// 不区分平台（旧命名 `moldingFoam-openfoam14-linuxArm64…` 与 `aarch64` / `x86_64`
/// 写法一并认，改名不至于下载不了）。无法识别的架构一律不匹配（不猜）。
pub fn bundle_asset_matches_arch(asset_name: &str, arch: &str) -> bool {
    let lower = asset_name.to_lowercase();
    let arm = ["arm64", "aarch64"];
    let x86 = ["amd64", "x86_64", "x86-64", "linux64"];
    match arch {
...
mod tests {
    ...
    #[test]
    fn bundle_asset_matches_current_release_naming() {
        // 现行命名 moldingFoam-<version>-<arch>.tar.xz（发布侧只出 Linux 包）
        for name in [
        ...
        for name in [
            ...
            "moldingFoam-openfoam14-linux64GccDPInt32Opt-20260913.tar.xz",
        ] {
            assert!(bundle_asset_matches_arch(name, "x86_64"), "{name}");
            assert!(!bundle_asset_matches_arch(name, "aarch64"), "{name}");
        }
