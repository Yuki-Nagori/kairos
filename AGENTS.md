...
| Rust 代码归属、模块组织、utils/错误/命名/测试/注释约定            | [ai-docs/rust-conventions.md](ai-docs/rust-conventions.md) |
...
| 去哪找评审 / 复现报告 / 上游往来                                  | [ai-docs/reviews/README.md](ai-docs/reviews/README.md)     |
...
- **目录职责**：`src-web/` 前端（TypeScript）；`src-crates/` Rust 领域层 crate（`kairos-core` 领域逻辑 + `kairos-cli` 无头 CLI）；`src-tauri/` Tauri 桌面适配层。各目录内部的 `src/` 是 Rust crate 固定结构，勿混淆。
- **依赖方向**：前端 `views / components → stores → api → utils / render`；Rust `src-tauri` / `kairos-cli` → `kairos-core`，core 内部 `error ← utils ← models / services`。`kairos-core` 禁止依赖 tauri；命令层不写业务逻辑，业务只住 core；`utils` 不认识领域类型（签名里出现网格/场/工况等词汇即出局）。
- **逻辑归属（Rust 优先）**：计算密集 / 数值 / 几何 / 场处理逻辑一律在 kairos-core 实现（TS 性能大部分场景不如 Rust，前端以 UI 编排为主）；TS 只做 UI 状态聚合与展示层逻辑，禁止重写 core 已有计算。判断口诀见 ai-docs/ARCHITECTURE.md §1.1。
